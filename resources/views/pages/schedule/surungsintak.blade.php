                    ...
                    <tbody>
                        @forelse ($schedules as $schedule)
                            <tr class="bg-white border-b hover:bg-green-50 transition-colors duration-200 cursor-pointer surung-row"
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-800">{{ $schedule->kecamatan }}</td>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const surungLocations = @json($surungLocations);
            let markers = {};

            if (surungLocations.length > 0) {
                const bounds = L.latLngBounds(surungLocations.map(loc => [loc.lat, loc.lng]));
                map.fitBounds(bounds, {
                    padding: [50, 50]
                });
            }

            surungLocations.forEach(loc => {
                ...
                const popupContent = `
                    <div class="w-64 rounded-lg overflow-hidden shadow-lg bg-white p-0">
                        <div class="p-4">
                            <div class="font-bold text-base mb-2 text-gray-800">${loc.area}</div>
                            <div class="space-y-1.5 text-xs text-gray-600">
                                <p>
                                    <span class="font-semibold">Kecamatan:</span> ${loc.kecamatan}
                                </p>
                                <p>
                                    <span class="font-semibold">Petugas:</span> ${loc.petugas}
                                </p>
                                <p>
                                    <span class="font-semibold">Deskripsi:</span> ${loc.deskripsi || 'Tidak ada deskripsi.'}
                                </p>
                            </div>
                        </div>
                    </div>`;
            ...
            const tableRows = document.querySelectorAll('.surung-row');
