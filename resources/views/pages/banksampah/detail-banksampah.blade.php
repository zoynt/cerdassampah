...
<div class="space-y-6">
        ...
        <div x-data="{
                ...
                get filteredBanks() {
                    ...
                    return this.banks.filter(
                        bank => bank.nama.toLowerCase().includes(this.search.toLowerCase())
                    );
                },
                selectBank(slug) {
                    window.location.href = '/bank-sampah/' + slug;
                }
            }" class="relative">
    ...
    <div class="relative h-48 rounded-2xl overflow-hidden shadow-lg">
        <img src="{{ $bankSampah->image_url }}" alt="Foto {{ $bankSampah->nama }}" class="w-full h-full object-cover">
        <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
        <div class="absolute bottom-0 left-0 p-6">
            <h1 class="text-3xl font-bold text-white">{{ $bankSampah->nama }}</h1>
            <p class="text-white/80">{{ $bankSampah->alamat }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg">
            ...
            {{-- ======================================================= --}}
            {{-- PERBAIKAN LOGIKA ACCORDION DI SINI --}}
            {{-- ======================================================= --}}
            <div x-data="{
                    openCategories: {{ json_encode($hargaSampah->keys()) }},
                    toggleCategory(kategori) {
                        if (this.openCategories.includes(kategori)) {
                            this.openCategories = this.openCategories.filter(item => item !== kategori);
                        } else {
                            this.openCategories.push(kategori);
                        }
                    }
                }"
                class="space-y-2">
                ...
                @forelse ($hargaSampah as $kategori => $items)
                    <div>
                        {{-- Tombol sekarang memanggil fungsi toggleCategory --}}
                        <button @click="toggleCategory('{{ $kategori }}')" class="w-full flex justify-between items-center p-4 bg-gray-100 rounded-lg text-left font-semibold text-gray-700 hover:bg-gray-200 transition">
                            <span>{{ $kategori }}</span>
                            {{-- Panah berputar jika kategori ada di dalam array openCategories --}}
                        ...
                        {{-- Konten ditampilkan jika kategori ada di dalam array openCategories --}}
                        <div x-show="openCategories.includes('{{ $kategori }}')" x-transition class="p-4 space-y-3">
                            @foreach ($items as $item)
                            <div class="flex justify-between items-center border-b pb-2">
                                <p class="text-gray-600">{{ $item->nama_item }}</p>
                                <p class="font-medium text-gray-800">Rp {{ number_format($item->harga, 0, ',', '.') }}/kg</p>
                            </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-lg">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex">
                        ...
                        <span class="text-gray-800">{{ $bankSampah->jam_operasional }}</span>
                    </div>
                    <div class="flex">
                        <span class="w-24 font-semibold text-gray-500 shrink-0">Kontak</span>
                        <span class="text-gray-800">{{ $bankSampah->kontak_person }} ({{ $bankSampah->nomor_telepon }})</span>
                    </div>
                    <div class="flex">
                        <span class="w-24 font-semibold text-gray-500 shrink-0">Deskripsi</span>
                        <span class="text-gray-800 flex-1">{{ $bankSampah->deskripsi }}</span>
                    </div>
                </div>
                <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {{-- Tombol Kontak dengan Ikon WhatsApp --}}
                    <a href="https://wa.me/{{ $bankSampah->nomor_telepon }}" target="_blank" class="flex items-center justify-center gap-2 text-center w-full py-2 bg-green-100 text-green-800 font-semibold rounded-lg hover:bg-green-200 transition">
                    ...
                    {{-- Tombol Lihat Rute dengan Ikon Peta --}}
                    <a href="https://www.google.com/maps/dir/?api=1&destination={{ $bankSampah->latitude }},{{ $bankSampah->longitude }}" target="_blank" class="flex items-center justify-center gap-2 text-center w-full py-2 bg-blue-100 text-blue-800 font-semibold rounded-lg hover:bg-blue-200 transition">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const lat = {{ $bankSampah->latitude }};
            const lng = {{ $bankSampah->longitude }};
            const map = L.map('map').setView([lat, lng], 15);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
            }).addTo(map);
            L.marker([lat, lng]).addTo(map)
                .bindPopup('<b>{{ $bankSampah->nama }}</b><br>{{ $bankSampah->alamat }}').openPopup();
        });
    </script>
@endpush
