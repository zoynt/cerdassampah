...
@push('styles')
    ...
    <style>
        ...
        /* Sembunyikan ikon panah segitiga bawaan Select2 */
        .select2-container--default .select2-selection--single .select2-selection__arrow b {
            display: none !important;
        }
        /* Ganti dengan ikon SVG, atur posisi, warna, dan transisi */
        ...
        /* Putar ikon saat dropdown dibuka */
        .select2-container--open .select2-selection--single .select2-selection__arrow {
            transform: rotate(180deg);
        }

        /* --- Animasi untuk Ikon Field Status Awal --- */
        
        /* Beri transisi pada ikon SVG di dalam wrapper */
        .select-wrapper svg {
            transition: transform 0.2s ease-in-out;
        }
        /* Saat select di dalamnya aktif/fokus, putar ikon SVG-nya */
...
<div class="space-y-6">
    ...
    <div x-data="{ isOpen: false }" class="bg-white rounded-xl shadow-lg">
        <div @click="isOpen = !isOpen" class="p-6 flex justify-between items-center cursor-pointer hover:bg-gray-100" :class="{ 'border-b': isOpen, 'rounded-xl': !isOpen, 'rounded-t-xl': isOpen }">
            <h2 class="text-lg font-bold text-gray-800">Tambah Jenis Sampah Baru</h2>
            ...
            <form action="{{ route('pengelola.harga.store') }}" method="POST">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="add-new-category" class="block mb-2 text-sm font-medium text-gray-700">Jenis Sampah</label>
                            <select id="add-new-category" name="waste_category_id" required style="width: 100%;">
                        ...
                        <div>
                            ...
                            <div class="relative"><div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none"><span class="text-gray-500">Rp</span></div><input type="text" id="price_per_kg_display" placeholder="2.000" class="price-input w-full h-11 pl-10 pr-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50" required><input type="hidden" name="price_per_kg" id="price_per_kg_value"></div>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="description" class="block mb-2 text-sm font-medium text-gray-700">Deskripsi (Opsional)</label>
                            <input type="text" name="description" placeholder="Cth: Botol bening bekas air mineral" class="w-full h-11 px-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50">
                        </div>
                        <div>
                ...
                <div class="px-6 py-4 bg-gray-50 flex justify-end rounded-b-xl">
                    <button type="submit" class="h-11 text-white bg-green-600 hover:bg-green-700 font-medium rounded-lg text-sm px-6 text-center flex items-center justify-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" /></svg><span>Tambah Item</span></button>
                </div>
            </form>
        </div>
    </div>
            ...
            <div id="item-{{ $product->id }}" class="bg-white rounded-xl shadow-lg transition-all duration-300">
                    ...
                    <div class="p-6 flex flex-col sm:flex-row sm:justify-between gap-4">
                        <div>
                            <div class="flex items-center gap-3">
                                <h3 class="text-xl font-bold text-gray-900">{{ $product->category->name ?? 'N/A' }}</h3>
                                @if($product->status == 'Aktif')<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Aktif</span>@else<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Tidak Aktif</span>@endif
                            </div>
                        ...
                        <div class="sm:text-right flex-shrink-0">
                            ...
                            <div class="mt-2 flex items-center sm:justify-end gap-2">
                                <button onclick="toggleEdit({{ $product->id }})" class="bg-blue-100 hover:bg-blue-200 text-blue-600 font-bold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors">
                                ...
                                <form id="delete-form-{{ $product->id }}" action="{{ route('pengelola.harga.destroy', $product->id) }}" method="POST" class="m-0">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete({{ $product->id }})" class="bg-red-100 hover:bg-red-200 text-red-600 font-bold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors" title="Hapus">
                ...
                <div class="edit-mode hidden">
                    <form action="{{ route('pengelola.harga.update', $product->id) }}" method="POST">
                        @csrf @method('PUT')
                        <div class="p-6"><h3 class="text-xl font-bold text-gray-900 mb-4">{{ $product->category->name ?? 'N/A' }}</h3><div class="space-y-4"><div><label class="block mb-1 text-sm font-medium text-gray-700">Harga Baru /kg</label><div class="relative"><div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none"><span class="text-gray-500">Rp</span></div><input type="text" data-target="price-edit-{{$product->id}}" value="{{ (int) $product->price_per_kg }}" class="price-input w-full h-11 pl-10 pr-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50" required><input type="hidden" name="price_per_kg" id="price-edit-{{$product->id}}" value="{{ (int) $product->price_per_kg }}"></div></div><div><label class="block mb-1 text-sm font-medium text-gray-700">Deskripsi</label><input type="text" name="description" value="{{ $product->description }}" class="w-full h-11 p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50"></div><div><label class="block mb-1 text-sm font-medium text-gray-700">Status</label><div class="relative"><select name="status" class="w-full h-11 pl-4 pr-10 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 appearance-none" required><option value="Aktif" @selected($product->status == 'Aktif')>Aktif</option><option value="Tidak Aktif" @selected($product->status == 'Tidak Aktif')>Tidak Aktif</option></select><div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none"><svg class="w-5 h-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg></div></div></div></div></div>
...
@push('scripts')
<script>
    // Menunggu seluruh dokumen siap (standar jQuery)
    $(document).ready(function() {
        // Inisialisasi Select2 pada elemen Anda
        $('#add-new-category').select2({
            placeholder: 'Ketik atau pilih kategori...',
            tags: true, // Mengizinkan pengguna untuk membuat opsi baru
            dropdownParent: $('body') // KUNCI: Menyelesaikan masalah dropdown terpotong
        });
        ...
        function setupPriceInput(displayInput) {
            ...
            const valueInputId = displayInput.dataset.target || displayInput.id.replace('_display', '_value');
