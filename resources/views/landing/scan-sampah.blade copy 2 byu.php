...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        }, 50); // Delay agar halaman termuat dengan baik
    ...
    function displayFileName() {
        ...
        const uploadedImage = document.getElementById('uploaded-image'); // Gambar yang di-upload

        if (fileInput.files.length > 0) {
            const file = fileInput.files[0];

            // Menyembunyikan teks nama file
            ...
            reader.onload = function(e) {
                uploadedImage.src = e.target.result;  // Set image source with the uploaded file
            };
            ...
            // Jika tidak ada file terpilih, tampilkan teks dan ikon upload
            fileName.textContent = '';  // Kosongkan teks nama file
            uploadText.style.display = 'block'; // Tampilkan teks upload lagi
            uploadIcon.style.display = 'block'; // Tampilkan ikon upload lagi
            uploadedImage.style.display = 'none'; // Sembunyikan gambar yang di-upload
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Example of data to be shown (this would come from your backend or scan result)
        const sampleScanResult = {
            imageUrl: 'path_to_image.jpg',  // Replace with dynamic image URL from scan result
            title: 'Botol Plastik',
            type: 'Anorganik',
            handling: [
    ...
    if (data && data.imageUrl) {
        ...
        scanTitle.textContent = data.title;
        scanType.textContent = `Jenis Sampah: ${data.type}`;

        // Display handling tips
        handlingTips.innerHTML = '';
        ...
        // Simulate scan result after file is uploaded
        fileInput.addEventListener('change', function () {
            if (fileInput.files.length > 0) {
                // Simulate that the file has been processed and a result is obtained
                showScanResult(sampleScanResult);
            }
        });
    });
    ...
    function displayFileName() {
        ...
        if (fileInput.files.length > 0) {
            ...
            reader.onload = function(e) {
                ...
                uploadedImage.style.display = 'block'; // Show the image once it's loaded
            };
    ...
    document.getElementById('scan-sampah-btn').addEventListener('click', function (e) {
        ...
        .then(response => {
            return response.json(); // Verifikasi jika respons adalah JSON
        })
        .then(data => {
            if (data.error) {
                alert(data.error);
    ...
    // Fungsi untuk menampilkan hasil scan di bawah form
    function showScanResult(data) {
        document.getElementById('scan-result').classList.remove('hidden'); // Tampilkan card hasil scan
        ...
        const handlingTips = document.getElementById('handling-tips');
        handlingTips.innerHTML = ''; // Kosongkan list sebelumnya
        data.handling.forEach(tip => {
            const li = document.createElement('li');
            li.textContent = tip;
            handlingTips.appendChild(li);
        ...
        const recyclingTips = document.getElementById('recycling-tips');
        recyclingTips.innerHTML = ''; // Kosongkan list sebelumnya
        data.recycling.forEach(tip => {
            const li = document.createElement('li');
            li.textContent = tip;
            recyclingTips.appendChild(li);
